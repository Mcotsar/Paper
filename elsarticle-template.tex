...
As the Table~\ref{table:variable} shows, the most differences were focused on the protruding rim and rim width 2.   
...
We compared morphometric and spatial distance by performing peer-to-peer analysis between all the workshops. We calculated the geographical distance between each site and the distance among amphora measurements, calculated using the previous results. The workshops were chosen with different distance in order to prove the correlation between spatial distance and variability of the amphorae.  Table~\ref{table:spatgeo} shows the results of the analysis of morphometric distance in comparison with the real geographic distance. Here morphometric distance of the amphorae are strongly correlated with the spatial distance of workshops. When the geographic distance is lower as the example of Belen and Malpica the morphometric distance is more similar (poner figura del grafico de las distancias). Thus, the results suggest a variability on the making-techniques processes might depend on the spatial distance.  
%aumentar con nuevos resultados 
\maria{debería poner el plot con los resultados del geographic distance y morphometric?}
